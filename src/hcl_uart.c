...
** Return value:        COM port handle if OKful, -1=NG
*****************************************************************************/
int uartInit(const char* comPortPath, int baudrate) {
  ...
  fd_serial = openComPort(comPortPath, baudRate);
  return fd_serial;
}
...
ComPortHandle openComPort(const char* comPortPath, speed_t baudRate) {
  ...
  // fd_serial = open(comPortPath, O_RDWR | O_NOCTTY | O_NDELAY); // O_NDELAY
  // means non-blocking I/O, results in unstable reads and writes

  if (port < 0)  // Opening of port NGed
  {
